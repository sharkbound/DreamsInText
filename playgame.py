...
class App(tk.Frame):
    def __init__(self, master=None):
        # in app - textbox contains output from print commands in other python files,
        # entry sends user input to other python files
        ...
        self.textbox = tk.Text(width=130)
        ...
        # pressing escape make the GUI close
        self.master.bind('<Escape>', lambda *args: self.quit())

        beginGame()

    def user_input(self, event):
        user_input = self.contents.get()
        # calls play_turn every time a user input / command is entered -
        # if play_turn returns False then reset in instance of game over has been called
